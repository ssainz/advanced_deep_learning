...
def generate_deep_net_normal_init(size_of_image = 784,
    ...
    # this matrix is shape [batch, (image_width - filter_size), (image_height - filter_size) , size_of_features_in_first_convolution]
    convolution_1_before_relu = tf.nn.conv2d(x_image, W_conv1, strides=[1, 1, 1, 1], padding=cur_padding)
    h_conv1 = tf.nn.relu(convolution_1_before_relu + b_conv1)
    ...
    # this matrix is size
    convolution_2_before_relu = tf.nn.conv2d(h_pool1, W_conv2, strides=[1, 1, 1, 1], padding=cur_padding)
    h_conv2 = tf.nn.relu(convolution_2_before_relu + b_conv2)
    ...
    return (train, loss, accuracy, x, y_hat, y, W_conv1, W_conv2, convolution_1_before_relu, convolution_2_before_relu)
...
def print_filters(filters):
    for filter_name in filters.keys():
        ...
        number_of_input_features = filter.shape[2]
        number_of_feature_maps = filter.shape[3]
        for _feature in range(number_of_feature_maps):
            feature_map = filter[:,:,:,_feature]
        ...
        title = "Filters used to generate feature maps for any given image %s" % (filter_name)
...
def print_convolutions(convolution):
    # convolution has shape = [batch_size, image_width, image_height, number_of_feature_maps]

    for convolution_name in convolutions.keys():
        # For each convolution, go over its feature maps:
        convolution = convolutions[convolution_name]
        # Just picks the first batch!
        convolution = convolution[0,:,:,:]
        feature_maps = None
        image_size_x = convolution.shape[0]
        image_size_y = convolution.shape[1]
        number_of_feature_maps = convolution.shape[1]
        first = True
        for _feature in range(number_of_feature_maps):
            feature_map = convolution[:,:,_feature]
            # Now we have one feature map. We append it to the feature maps of this column
            if first:
        ...
        # Printing the convolutions
        subp = plt.subplot(1, 1, 1)
        title = "Feature maps of random image after filters have been applied %s" % (convolution_name)
        subp.set_title(title)
        subp.imshow(feature_maps)
        plt.draw()
        plt.show()

# First prepare the network
train, loss, accuracy, x, y_hat, y, W_conv1, W_conv2, conv_1_before_relu, conv_2_before_relu = generate_deep_net_normal_init(size_of_image = 3072,
...
print("Print convolutions (feature maps) when applied to random image")
random_image_index = np.random.randint(0,number_of_samples-1)
x_raw_tensor_one_image = cifar_from_vector_to_tensor(images[b'data'][random_image_index:random_image_index+1])
y_raw_vector_one_image = cifar_from_categorical_to_vector(images[b'labels'][random_image_index:random_image_index+1])

print("Print the random image")
random_image = np.reshape(x_raw_tensor[random_image_index], (32,32,3))
subp = plt.subplot(1,1,1)
title = "Random image (index %s)" % random_image_index
subp.set_title(title)
subp.imshow(random_image)
plt.draw()
plt.show()

[conv_1_before_relu, conv_2_before_relu] =  sess.run([conv_1_before_relu, conv_2_before_relu], feed_dict={x: x_raw_tensor_one_image, y_hat: y_raw_vector_one_image})
convolutions = {"First layer": conv_1_before_relu, "Second layer": conv_2_before_relu}
print_convolutions(convolutions)

print("Print filters used to generate all feature maps of any given image")
x_raw_tensor = cifar_from_vector_to_tensor(images[b'data'])
y_raw_vector = cifar_from_categorical_to_vector(images[b'labels'])
...
# print filters
filters = {"First layer": final_W_conv1, "Second layer": final_W_conv2}
